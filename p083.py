def find_minimal_sum(matrix):
    ...
    visited.extend((n, -1) for n in range(size))
...
def test_p083():
    assert find_minimal_sum(matrix_from_file('p081_test_matrix.txt')) == 2297


def p083():
    return find_minimal_sum(matrix_from_file('p083_matrix.txt'))
